...
코드는 결국 여러 개발자들의 협업으로 이루어진다. 따라서 나와 다른 개발자들은 작성한 서로의 코드를 보고 작업해야 한다.
이번 장에서는 코드의 중요한 세부사항을 다른 개발자들에게 전달하기 위한 몇가지 기법을 소개한다.

## 자신의 코드와 다른 개발자의 코드

다른 개발자들이 활발하게 코드를 변경하더라도 코드의 품질이 유지되려면 그만큼 코드가 튼튼하고 사용하기 쉬워야 한다. 고품질 코드를 작성할 때 가장 중요한 것은 다른 개발자가 코드를 사용할 때 발생할 수 있는 문제가 있는지, 문제가 있다면 이를 어떻게 해결할 것인지 선제적으로 조치를 취하는 것이다.

1. 자신에게 분명하다고 해서 다른 사람에게도 분명한 것은 아니므로 의도가 명확하게 코드를 짜야한다.
2. 다른 개발자는 코드 병합 과정 또는 히스토리에 대한 낮은 이해로 인해 의도치 않게 당신의 코드를 망가뜨릴 수 있다.
3. 시간이 지나면 자신의 코드를 기억하지 못한다. 이해가 쉬운 코드를 짜는 것은 다른 개발자 뿐만 아니라 자기 자신에 대한 배려이기도 하다.

## 내가 작성한 코드의 사용법을 다른 사람이 어떻게 알까?

- 함수, 클래스, 변수 등의 네이밍
- 함수나 생성자의 매개변수 또는 리턴 타입
- 함수 또는 클래스의 문서 또는 주석문: 문서를 이해하지 못하거나 최신화 되어있지 않을 가능성도 있음
- 코드 작성자에게 직접 묻기: 코드 작성자의 부재 또는 본인도 기억하지 못할 가능성 있음
- 함수나 클래스의 자세한 내부 구현 코드 분석하기: 시간적으로 효율적이지 않음

## 코드 계약

서로 다른 코드 간의 상호과정을 하나의 계약으로 취급하여, 호출하는 코드는 특정 요건을 충족해야만 하고 호출되는 코드는 요구하는 리턴값을 반환하거나 데이터 일부를 수정해야 한다.
모든 것이 계약에서 정의되므로 분명하지 않은 것이 없고 예상과 다르게 실행되는 것도 없다.

- 명확한(기본적인) 부분: 함수나 클래스의 이름, 파라미터 타입, 리턴 타입, 검사 예외(검사 패스하지 못하면 컴파일 x)
- 세부 조항: 주석문과 문서, 비검사 예외

다른 개발자가 코드를 올바르게 사용하기 위해 세부 조항에 의존하기 보다는 잘못된 일은 애초에 불가능하도록 설계하는 것이 좋다. 즉, 명백한 사항으로 바꾸는 것이다.

## 체크 및 어서션

컴파일러를 사용하여 계약을 확인하는 것 대신 런타임 검사를 활용할 수 있다. 이 방법은 컴파일러를 사용하여 계약을 강제할 수 없는 경우에 사용할 수 있다.
